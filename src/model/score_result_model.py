...
def create():
    ...
    classifier = classifier_utils.create(feature_columns)

    # Train the Model.
    classifier.train(
        input_fn=lambda:score_result_dataset.train_input_fn(train_x, train_y,100),steps=1000)

    # Evaluate the model.
    eval_result = classifier.evaluate(
        input_fn=lambda:score_result_dataset.eval_input_fn(test_x, test_y,100))
